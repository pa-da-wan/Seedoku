...
def get_perspective_transform(image, source_pts, width, height):
    destination_coordinates = np.array([[0, 0], [width, 0], [0, height], [width, height]])
    xform_matrix = cv2.getPerspectiveTransform(np.float32(source_pts), np.float32(destination_coordinates))
...
def display_nums(image, number_list, text_color=(140, 230, 0)):
    """
